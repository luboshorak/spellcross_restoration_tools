...
SpellData::SpellData(wstring &data_path,wstring& cd_data_path,wstring& spec_path,std::function<void(std::string)> status_list,std::function<void(std::string)> status_item)
{
	...
	// load aux 7pix font (optional)
	if (status_list) status_list("Loading aux 7-pix font...");
	std::filesystem::path font7_path = std::filesystem::path(spec_path) / "font_spellcross_7pix.fnt";

	try {
		font7 = new SpellFont(font7_path.wstring());
		if (status_list) status_list(" - OK");
	}
	...
	if (status_list) status_list("Loading aux 14-pix font...");
	std::filesystem::path font14_path = std::filesystem::path(spec_path) / "font_spellcross_14pix.fnt";

	try {
		SpellFont font14_aux(font14_path.wstring());
		if (font) font->Merge(font14_aux);
		if (status_list) status_list(" - OK");
	}
	catch (const std::exception& error) {
		if (status_list) status_list(" - failed! (optional, continuing)");
	...
	for (auto& terr : terrain) {
		terr->font = font;
		terr->font7 = font7; // may be nullptr, code using it must handle that
	}

...
FormVideoBox::FormVideoBox(wxPanel* parent,wxWindowID win_id,SpellData* spell_data,std::string name,int zoom)
{
    ...
    const std::string entry_name = std::filesystem::path(name).filename().string();
    ...
    if (ext == ".DP2")
    {
        ...
        candidates.push_back((std::filesystem::path(m_spelldata->cd_data_path) / "SPEAKER.FS").wstring());
        candidates.push_back((std::filesystem::path(m_spelldata->data_path) / "SPEAKER.FS").wstring());
        // common dev/run layouts
        candidates.push_back((std::filesystem::path("temp") / "SPEAKER.FS").wstring());
    }
    else
    {
        // all other video stuff should be in MOVIE.FS
        candidates.push_back((std::filesystem::path(m_spelldata->cd_data_path) / "MOVIE.FS").wstring());
        candidates.push_back((std::filesystem::path(m_spelldata->data_path) / "MOVIE.FS").wstring());
        // common dev/run layouts
        candidates.push_back((std::filesystem::path("temp") / "MOVIE.FS").wstring());
        candidates.push_back((std::filesystem::path("temp") / "MOVIE" / "MOVIE.FS").wstring());
        candidates.push_back(std::filesystem::path("MOVIE.FS").wstring());
    }

    for (const auto& c : candidates)
    {
        if (std::filesystem::exists(c)) { archive_path = c; break; }
    }

    if (archive_path.empty())
    {
        // archive not found
        throw runtime_error(string_format("Video file \"%s\" not found (missing archive).", entry_name.c_str()));
    }
    try {
        ...
        // try get video file
        m_video = new SpellVideo(&fs, entry_name);

    }catch(const runtime_error& error) {
        throw runtime_error(string_format("Loading video file \"%s\" failed (%s)!", entry_name.c_str(), error.what()));
    }

...
bool SpellMap::EnemyTurnStep()
{
    ...
    while (enemy_turn_idx < enemy_turn_list.size())
    {
        ...
        // select this unit internally (enemy units are not selectable via UI)
        unit_selection = enemy;
        unit_selection_mod = true;

        // 1) try attack: compute attack range for this unit synchronously
        ...
        // 2) otherwise move towards nearest alliance unit (melee will get into contact this way)
        ...
        // choose best reachable neighbor tile around the target
        MapXY best_tile;
        best_tile.Clear();
        ...
        {
            MapXY t = GetNeighborTile8D(nearest->coor, dir);
            if (!t.IsSelected())
                continue;

            // tile occupancy check (same altitude class blocks)
            auto pxy = ConvXY(t);
            ...
            auto path = unit_range->FindPath(enemy, t);
            if (path.empty())
                continue;

            int cost = path.back().g_cost;
            if (cost <= enemy->action_points && cost < best_cost)
            {
                best_cost = cost;
                best_tile = t;
            }
        ...
        // nothing to do for this unit -> continue to next
    }

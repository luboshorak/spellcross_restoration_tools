...
namespace scsave
{
	...
	{
		static_assert(std::is_trivially_copyable_v<T>, "scsave::write expects POD/trivial type");
	...
	{
		static_assert(std::is_trivially_copyable_v<T>, "scsave::read expects POD/trivial type");
	...
	{
		uint32_t n = 0;
		if (!read(is, n)) return false;
		s.assign(n, '\0');
		if (n) is.read(&s[0], n);
		return bool(is);
	}

	// JSON-ish: "key": "VALUE" (hlavika nen strict JSON kvli backslashm)
...
// MapUnitType <-> string (MapUnitType je wrapper s GetString())
static std::string MapUnitTypeToString(MapUnitType& t)
{
	return t.GetString();
}
...
// jednotn zpis/ten MapUnit bez pointer
static void scsave_write_unit(std::ostream& os, SpellData* data, MapUnit* u, SpellMapEvents* evt_ctx)
{
	scsave::write(os, (int32_t)u->id);
	scsave::write(os, (int32_t)(u->unit ? u->unit->type_id : -1));

	scsave::write(os, (int32_t)u->coor.x);
	scsave::write(os, (int32_t)u->coor.y);

	scsave::write(os, (int32_t)u->experience);
	...
	scsave::write(os, (uint8_t)u->hide);
	scsave::write(os, (uint8_t)u->was_moved);
	scsave::write(os, (uint8_t)u->is_event);
	scsave::write(os, (uint8_t)u->was_seen);
	scsave::write(os, (int32_t)u->is_visible);

	scsave::write(os, (uint8_t)u->radar_up);
	scsave::write(os, (int32_t)u->altitude);
	scsave::write(os, (int32_t)u->azimuth);
	scsave::write(os, (int32_t)u->azimuth_turret);
	scsave::write(os, (int32_t)u->frame);

	scsave::write_string(os, MapUnitTypeToString(u->spec_type));
	scsave::write_string(os, MapUnitTypeToString(u->behave));
	...
	int32_t parent_id = u->parent ? (int32_t)u->parent->id : -1;
	int32_t child_id = u->child ? (int32_t)u->child->id : -1;
	int32_t creator_evt = (u->creator_event && evt_ctx) ? evt_ctx->GetEventID(u->creator_event) : -1;

	scsave::write(os, parent_id);
	scsave::write(os, child_id);
...
static bool scsave_read_unit(std::istream& is, SpellMap* map, SpellData* data, MapUnit*& out_u, ScsaveUnitLink& out_link)
{
	...
	uint8_t radar_up = 0;
	int32_t altitude = 0, azim = 0, azim_tur = 0, frame = 0;
	...
	if (!scsave::read(is, radar_up) || !scsave::read(is, altitude) || !scsave::read(is, azim) || !scsave::read(is, azim_tur) || !scsave::read(is, frame)) return false;
	...
	int32_t parent_id = -1, child_id = -1, creator_evt = -1;
	if (!scsave::read(is, parent_id) || !scsave::read(is, child_id) || !scsave::read(is, creator_evt)) return false;

	MapUnit* u = new MapUnit(map);
	u->id = id;
	u->unit = (data && data->units) ? data->units->GetUnit(type_id) : NULL;

	u->coor.x = x; u->coor.y = y;
	...
	u->is_enemy = is_enemy;
	u->hide = hide;
	u->was_moved = was_moved;
	u->is_event = is_event;
	u->was_seen = was_seen;
	u->is_visible = is_visible;

	u->radar_up = radar_up;
	u->altitude = altitude;
	u->azimuth = azim;
	u->azimuth_turret = azim_tur;
	u->frame = frame;

	u->spec_type = MapUnitTypeFromString(spec_s);
	u->behave = MapUnitTypeFromString(beh_s);
	...
	// Opraven kd pro std::string:
	u->name.clear();
	u->name = name;

	out_link.parent_id = parent_id;
	out_link.child_id = child_id;
	out_link.creator_event_idx = creator_evt;

	out_u = u;
	return true;
}

// --- v8: track when attack_map needs a synchronous refresh for current selected unit
static MapUnit* g_attack_map_dirty_for = nullptr;
...
int SpellMap::SaveGameStateToFile(const std::wstring& path)
{
	if (!IsLoaded() || !saves)
		return 1;
	...
	std::ofstream f(std::filesystem::path(path), std::ios::out | std::ios::binary | std::ios::trunc);
	if (!f.good())
		return 2;
	...
	// --- TILES ---
	{
		uint32_t n = (uint32_t)tmp.tiles.size();
		scsave::write(f, n);
		for (auto& t : tmp.tiles)
		{
			std::string l1 = (t.L1 ? std::string(t.L1->name) : "");
			std::string l2 = (t.L2 ? std::string(t.L2->name) : "");
			scsave::write_string(f, l1);
			scsave::write_string(f, l2);
			scsave::write(f, (int32_t)t.elev);
			scsave::write(f, (uint8_t)t.flags);
			scsave::write(f, (int32_t)t.hp);
		}
	}

	// --- UNITS ---
	{
		uint32_t n = (uint32_t)tmp.units.size();
		scsave::write(f, n);

		for (auto* u : tmp.units)
			scsave_write_unit(f, spelldata, u, tmp.events);
	...
	// --- EVENTS ---
	{
		auto& evts = tmp.events->GetEvents();
		uint32_t ecnt = (uint32_t)evts.size();
		scsave::write(f, ecnt);

		for (auto* e : evts)
		{
			scsave::write(f, (int32_t)e->evt_type);
			scsave::write(f, (uint8_t)e->is_objective);
			scsave::write(f, (int32_t)e->probability);
			scsave::write(f, (uint8_t)e->is_done);
			scsave::write(f, (uint8_t)e->hide);
			scsave::write(f, (int32_t)e->trig_unit_id);
			scsave::write(f, (int32_t)e->position.x);
			scsave::write(f, (int32_t)e->position.y);

			scsave::write_string(f, e->video);

			// texts: SpellMapEventMessageRec { text, is_done }
			{
				uint32_t tcnt = (uint32_t)e->texts.size();
				scsave::write(f, tcnt);
				for (auto& msg : e->texts)
				{
					std::string text_name = msg.text ? msg.text->name : "";
					scsave::write_string(f, text_name);
					scsave::write(f, (uint8_t)(msg.is_done ? 1 : 0));
				}
			}

			// event units: SpellMapEventUnitRec { unit, is_placed }
			{
				uint32_t ucnt = (uint32_t)e->units.size();
				scsave::write(f, ucnt);
				for (auto& eu : e->units)
				{
					scsave::write(f, (uint8_t)(eu.is_placed ? 1 : 0));
					scsave_write_unit(f, spelldata, eu.unit, tmp.events);
				}
			}
		}
	}
	...
	// --- UNITS_VIEW ---
	{
		uint32_t n = (uint32_t)tmp.units_view.size();
		scsave::write(f, n);
...
static bool ExtractQuotedField(const std::string& text, const char* key, std::string& out)
{
	...
	// najdi prvn uvozovku po dvojtece
	p = text.find('"', p);
	if (p == std::string::npos) return false;
	p++;
...
int SpellMap::LoadGameStateFromFile(const std::wstring& path)
{
	if (!saves)
		return 1;

	std::ifstream f(std::filesystem::path(path), std::ios::in | std::ios::binary);
	if (!f.good())
	{
	...
	size_t mpos = all.find(marker1);
	size_t mlen = marker1.size();
	if (mpos == std::string::npos)
	{
		mpos = all.find(marker2);
		mlen = marker2.size();
	}

	std::string header = (mpos == std::string::npos) ? all : all.substr(0, mpos);
	std::string payload = (mpos == std::string::npos) ? std::string() : all.substr(mpos + mlen);

	std::string map_path_s;
	if (!scsave::ExtractQuotedField2(header, "map_path", map_path_s))
	{
		last_error = "Save header missing map_path.";
		return 3;
	}

	std::wstring map_path_w(map_path_s.begin(), map_path_s.end());
	if (Load(map_path_w, spelldata))
	{
		last_error = "Failed to load base map from map_path.";
		return 4;
	}

	if (payload.empty())
	{
		last_error = "Save payload is empty.";
		return 5;
	}
	...
	if (!scsave::read(is, magic) || !scsave::read(is, ver) || magic != scsave::MAGIC)
	{
	...
	// --- TILES ---
	{
		...
		for (uint32_t i = 0; i < n; i++)
		{
			...
			if (!scsave::read_string(is, l1) || !scsave::read_string(is, l2)) return 9;
			if (!scsave::read(is, elev) || !scsave::read(is, flags) || !scsave::read(is, hp)) return 9;

			tmp.tiles[i].L1 = l1.empty() ? NULL : terrain->GetSprite(l1.c_str());
			tmp.tiles[i].L2 = l2.empty() ? NULL : terrain->GetSprite(l2.c_str());
			tmp.tiles[i].elev = elev;
			tmp.tiles[i].flags = flags;
			tmp.tiles[i].hp = hp;
		}
	}
	...
	{
		uint32_t n = 0;
		if (!scsave::read(is, n)) return 10;

		tmp.units.reserve(n);
		std::vector<ScsaveUnitLink> links;
		links.reserve(n);
		...
		// events container (potebujeme kvli creator_event relinku)
		tmp.events = new SpellMapEvents(this);

		// --- EVENTS ---
		uint32_t ecnt = 0;
		if (!scsave::read(is, ecnt)) return 13;

		for (uint32_t ei = 0; ei < ecnt; ei++)
		{
			auto* e = new SpellMapEventRec(this);

			int32_t evt_type = 0, prob = 100, trig_uid = -1, px = 0, py = 0;
			uint8_t is_obj = 0, is_done = 0, hide = 0;

			if (!scsave::read(is, evt_type) ||
				!scsave::read(is, is_obj) ||
				!scsave::read(is, prob) ||
				!scsave::read(is, is_done) ||
				!scsave::read(is, hide) ||
				!scsave::read(is, trig_uid) ||
				!scsave::read(is, px) ||
				!scsave::read(is, py))
				return 14;

			e->evt_type = (SpellMapEventRec::EvtTypes)evt_type;
			...
			if (!scsave::read_string(is, e->video)) return 14;

			// texts
			{
				uint32_t tcnt = 0;
				if (!scsave::read(is, tcnt)) return 14;
				e->texts.clear();
				for (uint32_t ti = 0; ti < tcnt; ti++)
				{
					std::string text_name;
					uint8_t done = 0;
					if (!scsave::read_string(is, text_name) || !scsave::read(is, done)) return 14;

					SpellTextRec* txt = text_name.empty() ? NULL : spelldata->texts->GetText(text_name.c_str());
					if (txt)
					{
						e->texts.emplace_back(txt);
						e->texts.back().is_done = (done != 0);
					}
				}
			}
			...
			{
				uint32_t ucnt = 0;
				if (!scsave::read(is, ucnt)) return 14;

				e->units.clear();
				for (uint32_t ui = 0; ui < ucnt; ui++)
				{
					uint8_t placed = 0;
					if (!scsave::read(is, placed)) return 15;
					...
					rec.unit = eu;
					rec.is_placed = (placed != 0);
					e->units.push_back(rec);
		...
		// relink parent/child + creator_event u hlavnch tmp.units
		for (size_t i = 0; i < tmp.units.size(); i++)
		{
			auto* u = tmp.units[i];
			if (links[i].parent_id >= 0) u->parent = tmp.GetUnit(links[i].parent_id);
			if (links[i].child_id >= 0) u->child = tmp.GetUnit(links[i].child_id);
			if (links[i].creator_event_idx >= 0) u->creator_event = tmp.events->GetEvent(links[i].creator_event_idx);
		}
	}
	...
	{
		uint32_t n = 0;
		if (!scsave::read(is, n)) return 16;
		tmp.units_view.resize(n);
		for (uint32_t i = 0; i < n; i++)
		{
			int32_t v = 0;
			if (!scsave::read(is, v)) return 16;
			tmp.units_view[i] = v;
		}
	}
	...
	if (path.rfind(L".DEF") != wstring::npos)
	{
		...
		for (int k = 0; k < mission_data->Size(); k++)
		{
			...
			if (cmd->name.compare("MissionMap") == 0)
			{
				...
				// patched:
				dta_path = (std::filesystem::path(map_path).parent_path() / std::filesystem::path(cmd->parameters->at(0))).wstring();
				dta_path += L".DTA";
			}
		}
		if (dta_path.empty())
...
if (!mission_data || !mission_data->Size())
{
	last_error = "MissionData section not found in DEF file!";
	delete def;
	Close();
	return(1);
}
...
void SpellMap::RelinkUnit(MapUnit* u, MapXY oldPos, MapXY newPos)
{
	if (!u || !oldPos.IsSelected() || !newPos.IsSelected())
		return;

	int oldIdx = ConvXY(oldPos);
	int newIdx = ConvXY(newPos);

	if (oldIdx == newIdx)
		return;

	// --- remove from old tile list ---
	MapUnit** pp = &Lunit[oldIdx];
	while (*pp)
	{
		if (*pp == u)
		{
	...
	// --- insert to new tile list (front) ---
	u->next = Lunit[newIdx];
	Lunit[newIdx] = u;

	// update logical coord too
	u->coor = newPos;
}
...
int SpellMap::SetBuffer(SpellTool& tool, int cycle)
{
	...
	for (auto& pos : rsel)
	{
		...
		MapSprite tile;
		if (tool_sprite->land_type == 0)
			tile.L2 = tool_sprite;
		else
			tile.L1 = tool_sprite;
		tile.flags = tool_sprite->GetMapFlags();
		tile.elev = 0;
		copy_buf.tiles.push_back(tile);
	}
	ReleaseMap();
...
// get selector tiles relative to center
std::vector<MapXY> SpellMap::GetRelSelection(TScroll* scroll)
{
	// default scroller
	if (!scroll)
		scroll = &scroller;
...
int SpellMap::Render(wxBitmap& bmp, TScroll* scroll, SpellTool* tool, std::function<void(void)> hud_buttons_cb)
{
	...
	// FIX: v game mode se renderuje jen to, co je ve view masce.
	// U nkterch map je view mask po startu przdn (vude 0) => ern obrazovka + HUD.
	// Pokud je maska przdn, doasn vypneme maskovn a mapu normln vykreslme.
	if (use_view_mask && unit_view)
	{
		bool any_visible = false;
		for (size_t k = 0; k < unit_view->view.size(); k++)
		{
			if (unit_view->view[k] != 0)
			{
				any_visible = true;
				break;
			}
	...
	// render projectile
	if (attacker && attacker->attack_state == MapUnit::ATTACK_STATE::FLIGHT && attack_pos.IsSelected() && target_pos.IsSelected() && attacker->unit && attacker->unit->hasProjectile(target))
	{
		// NOTE: do not use 'unit' here (it is the tile-loop iterator and may be NULL).
		double angle = attacker->unit_angle;
		if (attacker->unit->projectile)
		{
			...
			if (glyph)
			{
				...
				int denom = (attacker->attack_proj_delay > 0) ? attacker->attack_proj_delay : 1;
				int mxx = attack_pos.x + dx * attacker->attack_proj_step / denom - glyph->x_size / 2;
				int myy = attack_pos.y + dy * attacker->attack_proj_step / denom - glyph->y_size / 2;
...
bool SpellMap::IsUnitBusy(MapUnit* unit)
{
    if (!unit)
        return(false);
    return(unit->move_state != MapUnit::MOVE_STATE::IDLE ||
        unit->attack_state != MapUnit::ATTACK_STATE::IDLE ||
        unit->action_state != MapUnit::ACTION_STATE::IDLE);
}

bool SpellMap::StartMove_NoRangeCheck(MapUnit* unit, MapXY target)
{
    if (!unit)
        return(false);
    ...
    // find path (FindPath is synchronous)
    auto path = unit_range->FindPath(unit, target);
    if (path.size() < 2)
        return(false);
    ...
    // build path
    unit->move_nodes.clear();
    for (int nid = 1; nid < (int)path.size(); nid++)
        unit->move_nodes.push_back(path[nid]);
...
bool SpellMap::StartAttack_NoHUD(MapUnit* attacker, MapUnit* target)
{
    if (!attacker || !target)
        return(false);

    // has some attacks left?
    if (!attacker->GetFireCount())
        return(false);

    // allowed attack target?
    if (!attacker->unit->canAttack(target->unit))
        return(false);


	// indirect fire: minimum range (>= 3 hexes) for ground targets
	if (attacker->unit && attacker->unit->isIndirectFire() && target->unit && target->unit->isLand())
	{
    ...
    attacker->frame = 0;
    attacker->frame_stop = frame_stop;
    attacker->attack_state = MapUnit::ATTACK_STATE::DIR;

    attacker->ResetTurnsCounter();
    return(true);
}
...
{
	if (!ptr) return false;
	for (auto* u : list) if (u == ptr) return true;
	return false;
}
...
bool SpellMap::EnemyTurnStep()
{
	...
	while (enemy_turn_idx < enemy_turn_list.size())
	{
		...
		// 1) try attack (only against targets within this unit's computed attack range)
		...
		for (auto* u : units)
		{
			...
			int dist = abs(enemy->coor.x - u->coor.x) + abs(enemy->coor.y - u->coor.y);
			if (dist < best_dist)
			{
				best_dist = dist;
				best_target = u;
		...
		// If landing is impossible on current tile (land unit stacked here), move to nearest landable neighbor of the target first.
		if (enemy->unit->isAir() && enemy->unit->isActionLand() && enemy->CanSpecAction())
		{
			// find adjacent player ground unit (melee trigger)
			MapUnit* adj = nullptr;
			for (auto* u : units)
			{
				if (!u || u->is_enemy) continue;
				if (!u->unit || !u->unit->isLand()) continue; // melee is against ground
			...
			if (adj)
			{
				// landing morph type (aircraft down -> par3 in action_params[2])
				auto morph_type = spelldata->units->GetUnit(enemy->unit->action_params[2]);

				// only land if after morph we could actually attack that adjacent unit
				if (morph_type && morph_type->canAttack(adj->unit))
				{
					auto has_land_unit = [&](MapXY t) -> bool
						{
							auto pxy = ConvXY(t); // ConvXY wants non-const ref in your codebase
					...
					if (can_land_here)
					{
						// perform the same sequence as OnHUDairLandTakeOff() (land branch)
					...
					// cannot land here -> move towards nearest landable neighbor of adj (then land next step)
					...
					for (int dir = 0; dir < 8; dir++)
					{
						...
						// advance as far as possible this turn towards land_tile
					...
					// if we couldn't find a landable neighbor (or couldn't move), fall through to aggro chase below
				}
			}
		}
		...
		if (enemy->ai_aggro_ttl > 0)
		{
			...
			// could not move -> keep aggro but do nothing this unit
			continue;
		}
...
int SpellMap::Attack(MapXY pos, int prefer_air)
{
	...
	// indirect fire: enforce minimum range (>= 3 hexes) for ground targets
	if (target && unit->unit && unit->unit->isIndirectFire() && target->unit && target->unit->isLand())
	{
		if (unit->coor.Distance(target->coor) <= 2)
			return(1);
	}
...
MapUnit* SpellMap::SelectUnit(MapUnit* new_unit, bool scroll_to)
{
	...
	// game mode: never allow placement-drag on selected units; also ensure player units are active
	if (isGameMode() && unit_selection)
	{
		unit_selection->in_placement = false;
		if (!unit_selection->is_enemy)
			unit_selection->ActivateUnit();
	}
...
int SpellMap::RenderHUD(uint8_t* buf, uint8_t* buf_end, int buf_x_size, MapXY* cursor, MapUnit* cursor_unit, std::function<void(void)> hud_buttons_cb)
{
	...
	for (int pid = 0; pid < 2; pid++)
	{
		...
		// Single-man units: "wounded" is HP damage (0..GetHP), not "wounded men".
		if (unit->unit->isSingleMan())
		{
			int hp_max = unit->unit->GetHP();
			if (hp_max < 1) hp_max = 1;

			int hp_wound = men_wound;
			...
			// show HP values in the 3 HUD numbers (max / current / wounded)
			men_max = hp_max;
			men_active = hp_cur;
			men_wound = hp_wound;

			// bar is always full length, only the green part shrinks with HP
			wound_pix = 112;
			health_pix = hp_cur * 112 / hp_max;
		}
		else
		{
			if (men_max < 1) men_max = 1;

			int total = men_active + men_wound;
			if (total < 0) total = 0;
			if (total > men_max) total = men_max;

			if (men_active < 0) men_active = 0;
			if (men_active > men_max) men_active = men_max;

			health_pix = men_active * 112 / men_max;
			wound_pix = total * 112 / men_max;
		}

		// clamp to bar width (protect HUD memory)
		health_pix = (std::max)(0, (std::min)(112, health_pix));
		wound_pix = (std::max)(0, (std::min)(112, wound_pix));
		...
		int dig_count = (std::max)(0, (std::min)(unit->dig_level, 6));
	...
	while (true)
	{
		...
		while (true)
		{
			...
			if (is_radar)
			{
				...
				// radar should reveal units too (not only tiles)
				if (!is_fire && !map->Lunit.empty())
				{
					MapUnit* u = map->Lunit[next_mxy];
					while (u)
					{
						// new opponent contact?
						if (u->is_visible < 2 && u->is_enemy != target->is_enemy)
						{
							seen_units.push_back(u->id);
							if (detect_events)
								new_contact = true;
						}

						// linked SeeUnit() events?
						if (detect_events)
						{
		...
		if (is_fire)
		{
			...
			// indirect fire: enforce minimum range (>= 3 hexes)
			if (target && target->unit && target->unit->isIndirectFire())
			{
				int x0 = ref_pos.x - 2; if (x0 < 0) x0 = 0;
				int x1 = ref_pos.x + 2; if (x1 > map->x_size - 1) x1 = map->x_size - 1;
				int y0 = ref_pos.y - 2; if (y0 < 0) y0 = 0;
				int y1 = ref_pos.y + 2; if (y1 > map->y_size - 1) y1 = map->y_size - 1;
				...
				for (int y = y0; y <= y1; ++y)
				{
					...
					{
						MapXY p(x, y);
						if (p.Distance(ref_pos) <= 2)
							units_view[map->ConvXY(p)] = 0;
					}
				}
			}
...
int SpellMap::CanUnitAttack(MapUnit* target)
{
	...
	// attacking unit (selected)
	auto attacker = GetSelectedUnit();
	if (!attacker)
		return(false);

	// only opposite side targets
	if (attacker->is_enemy == target->is_enemy)
		return(false);

	auto pos = ConvXY(target->coor);

	// within range? (attack_map is computed for the selected unit)
	if (!unit_view->attack_map[pos])
		return(false);
	...
	else if (unit->attack_state != MapUnit::ATTACK_STATE::IDLE)
	{
		...
		else if (unit->attack_state == MapUnit::ATTACK_STATE::SHOT && tick_30ms)
		{
			...
			if (unit->frame >= unit->frame_stop || !unit->in_animation)
			{
				...
				// get fire origin (SAFE)
				FSU_resource::Txy fire_org{ 0, 0 };

				if (unit->in_animation &&
					unit->azimuth >= 0 &&
					unit->azimuth < unit->in_animation->anim.azimuths)
				{
					...
					unit->unit->gr_aux &&
					slope_id >= 0 &&
					slope_id < unit->unit->gr_aux->stat.slopes &&
					unit->azimuth_turret >= 0 &&
					unit->azimuth_turret < unit->unit->gr_aux->stat.azimuths)
				{
					fire_org =
						unit->unit->gr_aux
						->stat.fire_origin[slope_id][unit->azimuth_turret];
				}
				else if (unit->unit->gr_base &&
					slope_id >= 0 &&
					slope_id < unit->unit->gr_base->stat.slopes &&
					unit->azimuth >= 0 &&
					unit->azimuth < unit->unit->gr_base->stat.azimuths)
				{
					fire_org =
						unit->unit->gr_base
						->stat.fire_origin[slope_id][unit->azimuth];
				}
		...
		else if (unit->attack_state == MapUnit::ATTACK_STATE::HIT && tick_40ms)
		{
			...
			{
				// for unit target
				if (!target->attack_hit_pnm || target->attack_hit_pnm->frames.empty())
				{
					// no hit animation available -> finish immediately
					unit->attack_state = MapUnit::ATTACK_STATE::DIE_INIT;
				...
				else
				{
					...
					if (target->attack_hit_frame >= (int)target->attack_hit_pnm->frames.size())
					{
						// hit anim done:
						unit->attack_state = MapUnit::ATTACK_STATE::DIE_INIT;
						target->attack_state = MapUnit::ATTACK_STATE::IDLE;
		...
		{
			// target can be NULL (e.g. cleanup earlier in the state machine)
			if (!target)
			{
				unit->attack_state = MapUnit::ATTACK_STATE::UPDATE;
			}
			else if (target->isDead() && target->unit && target->unit->gr_die)
			{
				// play die action
...
int SpellMap::PlaceUnit(MapUnit* unit)
{
	...
	if (isGameMode())
	{
		unit->ActivateUnit();
		unit->ResetAP();
		unit->was_moved = true; // force range/view refresh
	}

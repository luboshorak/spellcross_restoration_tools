...
def _ascii6_ratio(b: bytes, sample_max: int = 200_000) -> float:
    """Heuristic: ASCII6 panels tend to use only top bits 00 or 01 (0x00..0x7F).
    Random data passes ~50%; panels are typically >>80%.
    """
    if not b:
        return 0.0
    s = b[:sample_max]
...
def _ascii6_decode(b: bytes) -> bytes:
    """Decode to 6-bit indices (0..63). Some files store indices as 0..63,
    some as 0x40..0x7F, and some mixed — taking the low 6 bits works for all."""
    return bytes((x & 0x3F) for x in b)
...
def _choose_panel_dims(dims: List[Tuple[int, int]]) -> Tuple[int, int]:
    ...
    #  - 575x480 (FULL panels like VMM_FULL.bin etc.)
    #  - 406x464 (BUY/UNITS/HIERARCH/RSRCH_BG)
    #  - 569x464 (STATS/OPTIONS/FACTORY)
    #  - 412x464 (INFO)
    def score(w: int, h: int) -> float:
        # hard-prefer exact known full-panel size
        if (w, h) == (575, 480):
            return 10_000.0

        s = 0.0
        # prefer common heights first
        s -= abs(h - 480) * 0.60
        s -= abs(h - 464) * 0.55

        # prefer common widths
        s -= abs(w - 575) * 0.18
        s -= abs(w - 569) * 0.12
        s -= abs(w - 406) * 0.30
        s -= abs(w - 412) * 0.12
...
def det_spellcross_ascii6_panel(b: bytes, path: str) -> Optional[Detection]:
    ...
    # Random data ~0.50 on this metric; real panels are typically >0.85
    if ratio < 0.80:
        return None
    ...
    # Confidence: ensure we beat RAW_IMAGE_GUESS (caps at 92)
    conf = int(90 + (ratio - 0.80) * 40)  # ratio=0.80 -> 90, ratio=1.00 -> 98
    if (w, h) in ((575, 480), (569, 464), (406, 464), (412, 464)):
        conf += 2
    conf = clamp(conf, 90, 99)

    details = f"Spellcross ASCII-6bit UI panel: {w}x{h} (top2bits_ratio={ratio:.3f})"
    ev = [
        f"size={n}",
        f"top2bits_ratio={ratio:.3f}",
        f"dims={w}x{h}",
        f"dim_candidates={len(dims)}",
        "decode=idx=(byte&0x3F)"
    ]
    return Detection(
        ...
        details=details,
        evidence=ev,
        convert_hint="Convert to PNG (ASCII-6bit panel). Uses matching <name>.PAL / SYSTEM.PAL if available. Exports gray + rotations + optional transparent.",
...
def analyze_bytes(b: bytes, path: str) -> FileReport:
    ...
    # If we have an ASCII6 panel detection, it's more specific than RAW_IMAGE_GUESS.
    has_ascii6 = any(d.kind == "SPELLCROSS_ASCII6_PANEL" for d in dets)
    if has_ascii6:
        for d in dets:
            if d.kind == "RAW_IMAGE_GUESS":
                # Keep it as a candidate, but never let it win.
                d.confidence = min(d.confidence, 89)

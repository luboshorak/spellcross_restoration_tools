...
def export_xlsx(reports: List[FileReport], out_path: str) -> None:
    ...
    for r in reports:
        ...
        ws.append([
            r.path, r.size, r.sha1, round(r.entropy, 3),
...
class App(tk.Tk):
    ...
    def _build_ui(self):
        ...
        # Tree view
        cols = ("type", "conf", "size", "entropy", "ext")
    ...
    def refresh_tree(self):
        ...
        for p in self.files:
            ...
            if rep:
                best = rep.best
                self.tree.insert("", tk.END, iid=p, values=(best.kind, best.confidence, rep.size, f"{rep.entropy:.3f}", best.ext))
            else:
                self.tree.insert("", tk.END, iid=p, values=("—", "—", os.path.getsize(p), "—", ""))

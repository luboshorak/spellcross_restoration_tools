...
def reorder_colmajor_to_rowmajor(img8: bytes, width: int, height: int) -> bytes:
    """Reorder pixels from column-major (x-major) to row-major (y-major).

    Some Spellcross raw screens look "otočené/roztažené" because pixels are laid out as:
        idx = x * height + y
    instead of the usual:
        idx = y * width + x
    """
    if len(img8) != width * height:
        raise ValueError("col-major reorder expects img8 size == w*h")
    out = bytearray(width * height)
    i = 0
    for x in range(width):
        col_off = x * height
        for y in range(height):
            out[y * width + x] = img8[col_off + y]
            i += 1
    return bytes(out)


def apply_transform(img8: bytes, width: int, height: int, transform: str) -> tuple[bytes, int, int]:
    """Apply simple geometric transforms to row-major indexed pixels."""
    t = (transform or "none").strip().lower()
    if t in ("none", "", "0"):
        return img8, width, height

    if len(img8) != width * height:
        raise ValueError("transform expects img8 size == w*h")

    if t in ("fliph", "flipx", "mirror", "h"):
        out = bytearray(width * height)
        for y in range(height):
            row = y * width
            for x in range(width):
                out[row + x] = img8[row + (width - 1 - x)]
        return bytes(out), width, height

    if t in ("flipv", "flipy", "v"):
        out = bytearray(width * height)
        for y in range(height):
            src = (height - 1 - y) * width
            dst = y * width
            out[dst:dst + width] = img8[src:src + width]
        return bytes(out), width, height

    if t in ("transpose", "swapxy", "t"):
        out = bytearray(width * height)
        for y in range(height):
            for x in range(width):
                out[x * height + y] = img8[y * width + x]
        return bytes(out), height, width

    if t in ("rot90cw", "cw", "r90"):
        # new_w = h, new_h = w
        out = bytearray(width * height)
        for y in range(height):
            for x in range(width):
                nx = height - 1 - y
                ny = x
                out[ny * height + nx] = img8[y * width + x]
        return bytes(out), height, width

    if t in ("rot90ccw", "ccw", "l90"):
        out = bytearray(width * height)
        for y in range(height):
            for x in range(width):
                nx = y
...
class App(tk.Tk):
    def __init__(self):
        ...
        self.order_var = tk.StringVar(value="row")
    ...
    def _build(self):
        ...
        ttk.Label(params, text="Memory order:").grid(row=2, column=0, sticky="w", pady=(8,0))
        ttk.Combobox(params, textvariable=self.order_var, width=10,
                     values=["row","col"], state="readonly")\
            .grid(row=2, column=1, sticky="w", padx=(6,18), pady=(8,0))
        ttk.Label(params, text="(col = x-major; když je obraz otočený/roztažený)")\
            .grid(row=2, column=2, columnspan=3, sticky="w", pady=(8,0))

        ttk.Label(params, text="Transform:").grid(row=3, column=0, sticky="w", pady=(8,0))
        ttk.Combobox(params, textvariable=self.transform_var, width=12,
                     values=["none","flipH","flipV","transpose","rot90cw","rot90ccw"], state="readonly")\
            .grid(row=3, column=1, sticky="w", padx=(6,18), pady=(8,0))
        ttk.Label(params, text="(aplikuje se po dekódování)").grid(row=3, column=2, columnspan=3, sticky="w", pady=(8,0))

        ttk.Label(params, text="Scale:").grid(row=4, column=0, sticky="w", pady=(8,0))
        ttk.Combobox(params, textvariable=self.scale_var, width=6,
                     values=["1","2","3","4","5","6","8"], state="readonly")\
            .grid(row=4, column=1, sticky="w", padx=(6,18), pady=(8,0))

        ttk.Label(params, text="Transparent idx:").grid(row=4, column=2, sticky="w", pady=(8,0))
        ttk.Entry(params, textvariable=self.transparent_var, width=18).grid(row=4, column=3, sticky="w", padx=(6,18), pady=(8,0))
        ttk.Label(params, text="(0 nebo 0,255 nebo 0-15)").grid(row=4, column=4, sticky="w", pady=(8,0))
    ...
    def reset(self):
        ...
        self.order_var.set("row"); self.transform_var.set("none")
    ...
    def do_convert(self):
        try:
            ...
            order = (self.order_var.get().strip() or "row").lower()
            transform = (self.transform_var.get().strip() or "none")
            ...
            # Fix resources stored as column-major (x-major) instead of row-major.
            if order == "col":
                img8 = reorder_colmajor_to_rowmajor(img8, w, h)

            # Optional post-decode transforms (useful when you are "close" but rotated/flipped).
            img8, w, h = apply_transform(img8, w, h, transform)
            ...
            if order == "col":
                extra.append("col")
            if transform.lower() not in ("none", "", "0"):
                extra.append(transform)
            extra_s = ("; " + ", ".join(extra)) if extra else ""
            self.status_var.set(f"OK: {out_path.name} ({w}×{h}, {fmt}){extra_s}")
